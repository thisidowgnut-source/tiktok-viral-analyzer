...
def render_vertical_video(
    ...
    """
    Composites 9:16 vertical video with burned-in kinetic subtitles.
    ...
    # Cache key including audio content, video content, and script
    video_stat = video_path.stat()
    audio_stat = audio_path.stat()
    cache_signature = f"{video_path.name}_{video_stat.st_size}_{video_stat.st_mtime}_{audio_path.name}_{audio_stat.st_size}_{script_text.strip()}"
    ...
    out_filename = f"render_{audio_stem}_{video_stem}_{content_hash}.mp4"
    ...
    # TikTok Yellow Caption styling: Arial bold, yellow fill, black thick border, bottom-centered
    filter_complex = (
        f"scale={target_width}:{target_height}:force_original_aspect_ratio=increase,"
        f"crop={target_width}:{target_height},"
        f"subtitles={rel_srt}:force_style='Fontname=Arial\\,Fontsize=22\\,Bold=1\\,PrimaryColour=&H0000FFFF\\,OutlineColour=&H00000000\\,BorderStyle=1\\,Outline=2.5\\,Shadow=1\\,Alignment=2\\,MarginV=140'"
    )
